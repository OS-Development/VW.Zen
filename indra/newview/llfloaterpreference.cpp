...
//	gSavedSettings.getControl("ChatFontSize")->getSignal()->connect(boost::bind(&LLViewerChat::signalChatFontChanged));
	...
	gSavedSettings.setBOOL("IMUseVerticalTabsControl", gSavedSettings.getS32("ChatWindow"));
...
void LLFloaterPreference::refreshDictLists(bool from_settings)
{
	...
	{
		const LLSD& dict = *dict_it;
		if ( (dict["installed"].asBoolean()) && (dict.has("language")) && (!dict["is_primary"].asBoolean()) &&
