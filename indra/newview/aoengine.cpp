...
{
	mEnabled=!mEnabled;
	gSavedPerAccountSettings.setBOOL("UseAO", mEnabled);
	enable(mEnabled);
}
...
void AOEngine::tick()
{
	const LLUUID categoryID=gInventory.findCategoryByName(ROOT_ZEN_FOLDER);

	if(categoryID.isNull())
	{
		llwarns << "no " << ROOT_ZEN_FOLDER << " folder yet. Creating ..." << llendl;
		gInventory.createNewCategory(gInventory.getRootFolderID(),LLFolderType::FT_NONE,ROOT_ZEN_FOLDER);
		mAOFolder.setNull();
	}
	else
	{
