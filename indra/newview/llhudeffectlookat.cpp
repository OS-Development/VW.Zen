...
void LLHUDEffectLookAt::packData(LLMessageSystem *mesgsys)
{
	...
	else //TODO: find out why this happens at all and fix there 
	{ 
	...
	if (!is_self) //TODO: find out why this happens at all and fix there 
	{ 
		LL_DEBUGS("HUDEffect")<< "Non-self Avatar HUDEffectLookAt message for ID: " << source_avatar->getID().asString() << LL_ENDL; 
		markDead(); 
		return; 
	} 
	else if (isLocalPrivate && is_private) // send nothing if we're not showing anything ourselves
	{
		markDead();
		return;
	}
	else if (is_private && target_type != LOOKAT_TARGET_AUTO_LISTEN) // spoof boring lookat target to others if we still want real local effects.
	{ 
