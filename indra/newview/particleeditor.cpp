...
#include "llinventoryfunctions.h"	// for ROOT_ZEN_FOLDER
...
{
	// TODO: move searching and creating the #ZEN folder into a helper class for all to use
	LLUUID categoryID;

	// first try to find the #ZEN folder
	categoryID=gInventory.findCategoryByName(ROOT_ZEN_FOLDER);

	// if no #ZEN folder was found, create one
	if(categoryID.isNull())
		categoryID=gInventory.createNewCategory(gInventory.getRootFolderID(),LLFolderType::FT_NONE,ROOT_ZEN_FOLDER);

	// if still no #ZEN folder was found, try to find the default "Scripts" folder
	if(categoryID.isNull())
	{
