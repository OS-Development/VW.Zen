...
bool const Base64::decode(const std::string &input, std::vector<unsigned char>& output)
{
	...
	output.push_back(((base64code0 << 2) | ((base64code1 >> 4) & 0x3)));
	...
	if(++i<input_length) {
		...
		output.push_back(((base64code1 << 4) & 0xf0) | ((base64code2 >> 2) & 0x0f));
	}
	...
	if(++i<input_length) {
		...
		output.push_back((((base64code2 << 6) & 0xc0) | base64code3 ));
	}
...
bool const Base64::encode(std::vector<unsigned char> const & input, std::string& output)
{
	unsigned int input_length=input.size();
	const unsigned char * input_ptr = input.data();
	return Base64::encode(input_length, input_ptr, output);
}

bool const Base64::encode(unsigned int input_length, const unsigned char * input_ptr, std::string& output)
{
